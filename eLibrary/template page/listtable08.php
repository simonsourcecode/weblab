...
<head>
<title>管理中心 - <?=empty($pagename)?"":$pagename;?> </title>
...
<div class="form-div">
  ...
  <input type="hidden" name="pagerows" value="<?=empty($pagerows)?10:$pagerows;?>" />
...
?>
<form method="post" action="?act=del" id="listForm" name="listForm">
<div class="list-div" id="listDiv">
<table width="600"  cellspacing="0" cellpadding="" border="1" border-collapse="collapse" id="list-table">
<tr>
...
if($counts){
?>	
<tr><td colspan="9"><table  border="1" cellspacing="0" cellpadding="0"> 
     <tr> 
     ...
     $pagezone = new Page($pagerows, $totalrows, $current_page, 5);  
     echo $pagezone->subPageCss2(); 
	  
      ?>
	  <script>
	  function goto_page(){var page_no=parseInt(document.getElementById('page_no').value);if(isNaN(page_no)||page_no<1||page_no>1){alert("页数必须为1-1");return;}window.location="http://localhost/eLibrary/template%20page/listtable08.php?page=$page_no";}
	  function input_page_no(){if(event.keyCode==13) goto_page();if(event.keyCode<47||event.keyCode>57) event.returnValue=false;}</script>
	  转到 第 <input type="text" size="3" class="SmallInput" name="page_no" id="page_no" onkeypress="input_page_no()" style='text-align:center;'> 页</td> 
     </tr> 
   </table></td></tr>
